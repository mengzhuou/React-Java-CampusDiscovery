...
public interface RsvpRepository extends JpaRepository<Rsvp, Long>{
    ...
    @Query(value = "SELECT e FROM Rsvp e WHERE e.event_id = ?1")
    List<Rsvp> getRsvp(long event_id);

    @Query(value = "SELECT e FROM Rsvp e WHERE e.event_id = ?1 AND e.status = ?2")
    List<Rsvp> getRsvpByStatus(long event_id, RsvpStatus status);

    @Query(value = "SELECT e FROM Rsvp e WHERE e.event_id = ?1 AND e.email = ?2")
    List<Rsvp> getRsvpEmail(long event_id, String email);

    @Modifying
    @Transactional
    @Query(value = "DELETE FROM Rsvp e WHERE e.event_id = ?1 AND e.email = ?2")
    void deleteRsvp(long event_id, String email);

    @Modifying
    @Transactional
    @Query(value = "UPDATE Rsvp e SET e.status = ?3 WHERE e.event_id = ?1 AND e.email = ?2")
